...
function uploadProjetoEdicao(array $arquivo): ?string
{
    ...
    if (($arquivo['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
        throw new RuntimeException('Erro ao enviar o arquivo do projeto.');
    }

    if (($arquivo['size'] ?? 0) > 10 * 1024 * 1024) {
        throw new RuntimeException('O arquivo do projeto não pode ultrapassar 10 MB.');
    }
    ...
    if (!isset($permitidos[$extensao])) {
        throw new RuntimeException('Formato de projeto inválido. Use PDF, PNG ou JPG.');
    }
    ...
    $nomeArquivo = 'projeto_' . bin2hex(random_bytes(16)) . '.' . $extensao;
    $destino = $diretorio . $nomeArquivo;

    if (!move_uploaded_file($arquivo['tmp_name'], $destino)) {
        throw new RuntimeException('Não foi possível salvar o novo projeto.');
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $enderecoObra = trim($_POST['endereco_obra'] ?? '');

    $valorTotal = limpaMoedaEdicao($_POST['valor_total'] ?? '0');
    $valorTerreno = limpaMoedaEdicao($_POST['valor_terreno'] ?? '0');
    $valorSubsidio = limpaMoedaEdicao($_POST['valor_subsidio'] ?? '0');
    $valorEntrada = limpaMoedaEdicao($_POST['valor_entrada'] ?? '0');

    if (!$clienteId || $enderecoObra === '' || $valorTotal <= 0) {
        ...
        if (!$stmtCliente->fetchColumn()) {
            ...
            /*
             * O valor_total representa o valor total do imóvel.
             * A construção corresponde ao valor total menos o terreno.
             */
            ...
            if ($sobraConstrucao <= 0) {
                $erro = 'O valor destinado à construção deve ser maior que zero.';
            } else {
                $arquivoProjeto = $obra['arquivo_projeto'];
                $novoArquivoProjeto = null;

                try {
                    if (
                        isset($_FILES['arquivo_projeto']) &&
                        ($_FILES['arquivo_projeto']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE
                    ) {
                        $novoArquivoProjeto = uploadProjetoEdicao($_FILES['arquivo_projeto']);
                        $arquivoProjeto = $novoArquivoProjeto;
                    }
                    ...
                    foreach ($etapas as $etapa) {
                        $novoValorEtapa = round(
                            ($sobraConstrucao * (float)$etapa['peso_percentual']) / 100,
                            2
                        );
                    ...
                    if (
                        ...
                        $obra['arquivo_projeto'] !== $novoArquivoProjeto
                    ) {
                        $arquivoAntigo = __DIR__ . '/../uploads/' . basename($obra['arquivo_projeto']);
                    ...
                    if (
                        $novoArquivoProjeto &&
                        is_file(__DIR__ . '/../uploads/' . $novoArquivoProjeto)
                    ) {
                        @unlink(__DIR__ . '/../uploads/' . $novoArquivoProjeto);
                    }
            ...
            <div class="row g-3">
                    ...
                    <select name="cliente_id" class="form-select" required>
                        ...
                        <?php foreach ($clientes as $cliente): ?>
                            <option
                                ...
                                <?= ((int)$cliente['id'] === (int)$obra['cliente_id']) ? 'selected' : '' ?>
                            >
                ...
                <div class="col-md-6">
                    ...
                    <input
                        ...
                        name="endereco_obra"
                        class="form-control"
                        value="<?= htmlspecialchars($obra['endereco_obra'], ENT_QUOTES, 'UTF-8') ?>"
                        required
                    >
                </div>
            </div>
            ...
            <div class="row g-3">
                <div class="col-md-6 col-lg-3">
                    ...
                    <input
                        ...
                        inputmode="numeric"
                        value="<?= number_format((float)$obra['valor_total'], 2, ',', '.') ?>"
                        required
                    >
                </div>
                ...
                <div class="col-md-6 col-lg-3">
                    ...
                    <input
                        ...
                        class="form-control mask-money"
                        inputmode="numeric"
                        value="<?= number_format((float)$obra['valor_terreno'], 2, ',', '.') ?>"
                    >
                </div>
                ...
                <div class="col-md-6 col-lg-3">
                    ...
                    <input
                        ...
                        class="form-control mask-money"
                        inputmode="numeric"
                        value="<?= number_format((float)$obra['valor_subsidio'], 2, ',', '.') ?>"
                    >
                </div>
                ...
                <div class="col-md-6 col-lg-3">
                    ...
                    <input
                        ...
                        class="form-control mask-money"
                        inputmode="numeric"
                        value="<?= number_format((float)$obra['valor_entrada'], 2, ',', '.') ?>"
                    >
                </div>
            ...
            <div class="row g-3">
                <div class="col-12">
                    <?php if (!empty($obra['arquivo_projeto'])): ?>
                        <div class="mb-2">
                            <span class="badge bg-light text-dark border p-2 me-2">
                                ...
                                <?= htmlspecialchars($obra['arquivo_projeto'], ENT_QUOTES, 'UTF-8') ?>
                            </span>

                            <a
                                href="/uploads/<?= rawurlencode(basename($obra['arquivo_projeto'])) ?>"
                                target="_blank"
                    ...
                    <small class="text-muted">
                        Selecione um novo arquivo somente se quiser substituir o projeto atual. Máximo: 10 MB.
                    </small>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function calcularSobra() {
        const valorTotal = parseMoeda(
            document.getElementById('valor_total').value
        );

        const valorTerreno = parseMoeda(
            document.getElementById('valor_terreno').value
        );

        const sobra = valorTotal - valorTerreno;

        const display = document.getElementById('sobra_display');

        display.innerText = formatarMoeda(Math.max(0, sobra));

        display.classList.toggle('text-danger', sobra <= 0);
        display.classList.toggle('text-success', sobra > 0);
    }
    ...
    [
        ...
        if (input) {
            input.addEventListener('input', calcularSobra);
        }
    });
